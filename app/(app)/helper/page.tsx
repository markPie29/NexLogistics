...
import { useTripStore, useFleetStore, useDriverStore, useHelperStore, useClientStore, useUiStore } from "@/lib/store";
import {
  Bell, ChevronLeft, ChevronRight, LayoutGrid, ClipboardList, Camera,
  Truck, Package, Navigation2, Clock3, CheckCircle2,
  MessageSquare, Wallet, Megaphone, X, MapPin,
} from "lucide-react";
import { toast } from "sonner";
import type { TripStatus, Trip } from "@/lib/types";
import { cn } from "@/lib/utils";
import { HelperNav } from "@/components/helper/HelperNav";
import type { HelperTab } from "@/components/helper/HelperNav";
import { HelperSidebar } from "@/components/helper/HelperSidebar";
import { MessagesView } from "@/components/driver/MessagesView";
...
type View = "dashboard" | "trip_details" | "trips_list" | "messages";
...
export default function HelperPage() {
  ...
  const [activeTab,     setActiveTab]    = useState<HelperTab>("dashboard");
  ...
  useEffect(() => {
    ...
    else if (v === "messages") { setView("messages"); setActiveTab("messages"); }
    else { setView("dashboard"); setActiveTab("dashboard"); }
  }, [searchParams]);

  // Resolve helper — find trips where helperId matches
  const helperId = user?.helperId ?? helpers[0]?.id;
  const myHelper = helpers.find((h) => h.id === helperId) ?? helpers[0];

  // Helper sees trips they're assigned to (via trip.helperId)
  const myTrips       = trips.filter((t) => t.helperId === helperId);
  ...
  const assignedDriver = drivers.find((d) => d.id === (activeTrip?.driverId ?? myHelper?.assignedDriverId));
  const unread        = notifications.filter((n) => !n.read).length;

  const hour = new Date().getHours();
  const greeting = hour < 12 ? "Good morning" : hour < 17 ? "Good afternoon" : "Good evening";
  const firstName = user?.name?.split(" ")[0] ?? myHelper?.name?.split(" ")[0] ?? "Helper";
  const fullName  = user?.name ?? myHelper?.name ?? "Roberto Lim";
  ...
  function submitStatus() {
    if (!activeTrip) return;
    setStatus(activeTrip.id, sheetStatus, `${fullName} (Helper)`, sheetNote || undefined);
    toast.success(`Status updated: ${STATUS_LABELS[sheetStatus]}`);
    setShowSheet(false);
    setSheetNote("");
  }

  function handleTab(tab: HelperTab) {
      ...
      <div className="space-y-5 pb-24">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-1">
                ...
                <span className="text-xs text-white/60">Online · Helper</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-4 gap-2">
            {([
              { label: "Assigned\nTrips",   value: myTrips.filter(t => !["completed","cancelled"].includes(t.status)).length || 0, icon: ClipboardList, color: "bg-blue-50 text-blue-600" },
              { label: "Completed\nToday",  value: completedTrips.length,  icon: CheckCircle2,  color: "bg-emerald-50 text-emerald-600" },
              { label: "Travelled\n(km)",   value: activeTrip?.distanceKm ?? 0,  icon: Navigation2,   color: "bg-amber-50 text-amber-600" },
              { label: "On Duty\n(hrs)",    value: "8.0",  icon: Clock3,  color: "bg-rose-50 text-rose-600" },
            ] as const).map((s) => (
          ...
          {activeTrip ? (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className="flex gap-3 items-stretch">
                  ...
                  <div className="text-right space-y-3 pt-0.5">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-sm text-brand-navy">{assignedDriver?.name?.split(" ")[0] ?? "—"}</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="px-4 pb-2 flex justify-between text-xs text-gray-400 border-t border-gray-50 pt-2">
                ...
                <span>Vehicle: <span className="font-semibold text-brand-navy">{vehicle?.plate ?? "—"}</span></span>
              </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center">
              <Package className="w-10 h-10 text-brand-teal mx-auto mb-3 opacity-40" />
              <p className="font-bold text-brand-navy">No active trip</p>
              <p className="text-sm text-gray-400 mt-1">You&apos;ll be notified when assigned to a trip.</p>
            </div>
          )}
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-4 gap-3">
            {([
              ...
              { label: "My\nDriver",      icon: Truck,         onClick: () => toast.info(assignedDriver ? `Driver: ${assignedDriver.name}` : "No driver assigned"),  badge: 0 },
              { label: "Messages",        icon: MessageSquare, onClick: () => { setView("messages"); setActiveTab("messages"); }, badge: 2 },
            ] as const).map((a) => (
              <button
                key={a.label}
                onClick={a.onClick}
        ...
        {/* Announcements */}
        <section>
          <h2 className="font-bold text-brand-navy text-sm mb-3">Announcements</h2>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-start gap-3">
            <div className="w-9 h-9 bg-amber-50 rounded-xl flex items-center justify-center shrink-0">
              <Megaphone className="w-4 h-4 text-amber-500" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm text-gray-700 leading-snug">Always assist the driver with cargo safety checks before departure.</p>
    ...
    return (
      <div className="space-y-4 pb-24">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          ...
          {client && <p className="text-xs text-gray-500 mt-2"><span className="font-medium text-gray-700">Client: </span>{client.name}</p>}
        </div>
                ...
                <div key={step} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={cn(
                      ...
                      done ? "border-brand-teal bg-brand-teal" : current ? "border-brand-teal" : "border-gray-300"
                    )}>
                  ...
                  <div className={cn("flex-1 pb-2", last ? "pb-0" : "")}>
                    <p className={cn("text-sm font-semibold leading-tight", done || current ? "text-brand-navy" : "text-gray-400")}>
                      {STATUS_LABELS[step]}
                    </p>
                    {log && <p className="text-[10px] text-gray-400 mt-0.5">{new Date(log.at).toLocaleDateString("en-PH",{month:"short",day:"numeric"})} • {new Date(log.at).toLocaleTimeString("en-PH",{hour:"2-digit",minute:"2-digit"})}</p>}
                  </div>
                </div>
              );
            })}
          ...
          <div className="space-y-3">
            {driver && (
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm text-brand-navy">{driver.name}</p>
    ...
    return (
      <div className="space-y-4 pb-24">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          ...
          {myTrips.length === 0 && <p className="text-sm text-gray-400 text-center py-8">No trips assigned yet.</p>}
          <div className="space-y-2">
            {myTrips.map((t) => (
              <button
                key={t.id}
              ...
              >
                <span className={cn("w-2.5 h-2.5 rounded-full shrink-0 mt-0.5",
                  t.status === "completed" ? "bg-emerald-500" : t.status === "in_transit" ? "bg-blue-500" : t.status === "cancelled" ? "bg-red-500" : "bg-amber-500"
                )} />
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm text-brand-navy">{t.id}</p>
  ...
  return (
    <div className="min-h-[100dvh] w-full flex flex-col bg-gray-50 overscroll-none">
      <header className="sticky top-0 z-30 bg-brand-navy w-full shrink-0" style={{ paddingTop: "env(safe-area-inset-top)" }}>
        <div className="max-w-lg mx-auto h-14 px-4 flex items-center justify-between">
          <button onClick={() => setSidebarOpen(true)} className="min-w-[44px] min-h-[44px] flex flex-col justify-center items-start gap-1.5 p-2 -ml-2" aria-label="Menu">
            <span className="block w-5 h-0.5 bg-white rounded" />
            <span className="block w-5 h-0.5 bg-white rounded" />
            <span className="block w-3.5 h-0.5 bg-white rounded" />
          </button>
          <div className="text-center leading-none select-none">
            <p className="text-white font-extrabold text-sm tracking-tight">NE<span className="text-brand-teal">X</span></p>
            <p className="text-[8px] tracking-[0.25em] text-brand-teal/80 font-semibold">LOGISTICS</p>
          </div>
          <button onClick={() => toast.info("Notifications")} className="relative min-w-[44px] min-h-[44px] flex items-center justify-center -mr-2" aria-label="Notifications">
            <Bell className="w-5 h-5 text-white" />
            {unread > 0 && <span className="absolute top-1 right-1 min-w-[16px] h-4 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-0.5">{unread > 9 ? "9+" : unread}</span>}
          </button>
        </div>
      </header>

      {view !== "dashboard" && (
        <div className="sticky top-14 z-20 bg-white border-b border-gray-100 shrink-0" style={{ top: `calc(3.5rem + env(safe-area-inset-top))` }}>
          <div className="max-w-lg mx-auto h-11 px-4 flex items-center">
            <button onClick={() => { setView("dashboard"); setActiveTab("dashboard"); setSelectedTrip(null); }} className="flex items-center gap-1 text-sm font-semibold text-brand-navy min-h-[44px]">
              <ChevronLeft className="w-4 h-4" />
              {view === "trip_details" ? "Trip Details" : view === "messages" ? "Messages" : "All Trips"}
            </button>
          </div>
        </div>
      )}
      ...
      <main className="flex-1 overflow-y-auto overscroll-contain">
        <div className="max-w-lg mx-auto px-4 pt-4">
          ...
          {view === "messages" && <MessagesView userName={fullName} />}
        </div>
      </main>

      <HelperNav active={activeTab} />
      <HelperSidebar isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} active={activeTab} />

      {/* Update Status Bottom Sheet */}
      {showSheet && activeTrip && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end">
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setShowSheet(false)} />
          <div className="relative bg-white rounded-t-3xl px-5 pt-3 w-full max-w-lg mx-auto shadow-2xl" style={{ paddingBottom: "max(env(safe-area-inset-bottom), 24px)" }}>
            <div className="w-10 h-1 bg-gray-200 rounded-full mx-auto mb-4" />
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-bold text-brand-navy">Update Trip Status</h3>
              <button onClick={() => setShowSheet(false)} className="min-w-[44px] min-h-[44px] -mr-2 rounded-full flex items-center justify-center" aria-label="Close">
                <span className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center"><X className="w-4 h-4 text-gray-500" /></span>
              </button>
            </div>
            <div className="space-y-2 mb-4">
              {SHEET_STATUSES.map(({ value, label }) => (
                <button
                  ...
                  className={cn("w-full flex items-center gap-3 p-4 rounded-xl border text-left transition-colors min-h-[56px]", sheetStatus === value ? "border-brand-teal bg-brand-teal/5" : "border-gray-100 bg-gray-50 active:bg-gray-100")}
                >
                  <div className={cn("w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0", sheetStatus === value ? "border-brand-teal" : "border-gray-300")}>
                    {sheetStatus === value && <span className="w-2.5 h-2.5 rounded-full bg-brand-teal" />}
                  </div>
                  <span className={cn("text-sm font-medium", sheetStatus === value ? "text-brand-teal" : "text-gray-600")}>{label}</span>
                </button>
              ))}
            </div>
            <textarea rows={3} maxLength={200} placeholder="Add notes (optional)" value={sheetNote} onChange={(e) => setSheetNote(e.target.value)} className="w-full text-sm border border-gray-200 rounded-xl px-3 py-3 resize-none focus:outline-none focus:ring-2 focus:ring-brand-teal/40 text-gray-700 placeholder:text-gray-400" />
            <p className="text-[10px] text-gray-400 text-right mb-4">{sheetNote.length}/200</p>
            <button onClick={submitStatus} className="w-full min-h-[56px] bg-brand-teal hover:opacity-90 active:scale-[0.98] text-white rounded-2xl font-bold text-sm flex items-center justify-center transition-all shadow-lg">
              Submit Update
            </button>
          </div>
        </div>
